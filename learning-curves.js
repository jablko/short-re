...
const data = [greedy, uniform, linear].map((policy) => {
    const [a, ...bs] = Array.from({ length: runs }, () => {
        ...
        let cummin = Infinity;
        return Array.from({ length: episodes }, () => (cummin = Math.min(cummin, agent.episode(policy).length)));
    });
    return a.map((value, i) => bs.reduce((cumsum, b) => cumsum + b[i], value) / (1 + bs.length));
});
...
const y = scaleLinear()
    .domain(data.reduce(([cummin, cummax], d) => [
...
const delaunay = new Delaunay(data.flatMap((d) => d.flatMap((value, i) => [x(i + 1), y(value)])));
...
svg
    ...
    .selectAll("text")
    .data(data.map((d, i) => d.reduce((largest, value, j) => {
    ...
    .text((d, i) => labels[i])
    .each(function (d) {
    const [cx, cy] = polygonCentroid(d.cell);
