  ...
  const debounce = setTimeout(async () => {
    ...
    try {
      const {
        data: from,
        ports: [agent],
      } = (await request(worker, values)) as MessageEvent<Agent["from"]>;
      ...
      if (Object.entries(from).length > 1) {
        ...
        for (let i = 0; i < episodes; i++) {
          const { data: pattern } = (await request(
            agent,
            undefined
        ...
        const startTo = from[""];
        const pattern = fromUnion(
          startTo[final as unknown as keyof typeof startTo]
        );
        patternOutput.replaceChildren(pattern);
      }
    } catch (e) {
...
function request<T>(responder: Worker<T> | MessagePort<T>, message: T) {
