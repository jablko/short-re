...
export class Agent {
  ...
  episode() {
    ...
    return (this.returns[this.cumpath] = fromUnion(
  ...
  step() {
    const startTo = this.from[""];
    const sources = Object.keys(startTo)
      .filter(
        (source) =>
    ...
    const sinks = Object.entries(this.from)
      .filter(([, sinkTo]) =>
  ...
  eliminateLinearSources(sources: (readonly [string, string[]])[]) {
    const startTo = this.from[""];
    return sources.reduce((cumsome, [source, children]) => {
      const { length: outdegree, 0: child } = children;
      if (outdegree > 1) return cumsome;
      startTo[child] = union(
        startTo[child] || [],
  ...
  eliminateLinearSinks(sinks: (readonly [string, string[]])[]) {
    return sinks.reduce((cumsome, [sink, parents]) => {
      const { length: indegree, 0: parent } = parents;
      if (indegree > 1) return cumsome;
      this.from[parent][
        this.final as unknown as keyof Agent["from"][keyof Agent["from"]]
      ] = union(
        this.from[parent][
...
function fromUnion(node: Union): string {
  return `(?:${node.map((child) => fromConcatenation(child)).join("|")})`;
}

function fromConcatenation(node: Concatenation) {
...
export const linear: Policy = function (choices) {
  const estimates = choices.map(([q, p]) => {
    const choice = `${this.cumpath!}${q}/${p}/`;
    return Object.entries(this.returns).reduce(
      (cummin, [path, pattern]) =>
        !path.startsWith(choice) ? cummin : Math.min(cummin, pattern.length),
      Infinity
    );
  });
